...
def find_optimal_beta_lagrange(dic_tf, num_qubits, objective, tol=1.0e-5, iter=10000, β_initial=None, bitstring_HF=None):
    assert objective in ['diagonal', 'mixed']  # TODO 'mixed'
    ...
    while True and iterations < iter:
        if objective == "diagonal":
            ...
            β_new, error = mixed_update_betas(dic_tf, num_qubits, influential_pairs, bitstring_HF, β_old)
        β_old = β_new
        iterations += 1
        if iterations % 100 == 0:
...
def mixed_lagrange_restriction_numerator(i, p, dict_tf, influential_pairs, bitstring_HF, β):
    assert p in ['X', 'Y', 'Z']
    ...
    for Q, R in influential_pairs:
        ...
        for j in range(len(Q)):
            if Q[j] == R[j] and Q[j] != "I":
                qubit = (len(Q)-1)-j  # qiskit ordering
                index = pauli_to_index[Q[j]]
                b = β[qubit][index]
                prod *= b**(-1)
            if Q[j] != R[j]:
                # then Q[i], R[i] are of the form I,Z or Z,I
                bit = int(bitstring_HF[j])
                m = (-1)**bit
                prod *= m
...
def mixed_lagrange_restriction_denominator(i, dict_tf, influential_pairs, bitstring_HF, β):
    pauli_to_index = {'X': 0, 'Y': 1, 'Z': 2}
    tally = 0.0
    for Q, R in influential_pairs:
        #if Q != R:
...
def mixed_lagrange_restriction(i, p, dic_tf, influential_pairs, bit_HF, β, denominator=None):
    ...
    if denominator is None:
        denominator = mixed_lagrange_restriction_denominator(i, dic_tf, influential_pairs, bit_HF, β)
